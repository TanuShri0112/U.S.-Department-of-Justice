    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Create, manage, and share upcoming events.</p>
        </div>
      ...
      <div className="bg-white border rounded-lg p-4 flex flex-col md:flex-row gap-3 items-stretch md:items-center">
        <div className="flex-1"><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by title or location..." /></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="border rounded-lg p-4 bg-white">
          <h3 className="font-medium mb-2">Manage RSVPs</h3>
          <p className="text-sm text-gray-600 mb-3">Track attendee responses and capacity.</p>
          <Button variant="outline" onClick={handleRsvps}>
            <Ticket className="h-4 w-4 mr-2" /> View RSVPs
          </Button>
        </div>
        <div className="border rounded-lg p-4 bg-white">
          <h3 className="font-medium mb-2">Quick Announce</h3>
          <p className="text-sm text-gray-600 mb-3">Send a quick event announcement.</p>
          <Button variant="outline" onClick={() => toast({ title: 'Announcement sent' })}>Send</Button>
        </div>
      </div>
        ...
        <ul className="divide-y">
          {filtered.map((e) => (
            <li key={e.id} className="p-4 flex items-center gap-4 hover:bg-gray-50">
              ...
              <div className="flex items-center gap-2">
                <Button size="sm" variant="outline" onClick={handleRsvps}>RSVPs</Button>
                <Button size="sm" variant="outline" onClick={() => handleShare(e)}><Share2 className="h-4 w-4 mr-1" /> Share</Button>
                <Button size="sm" variant="outline" onClick={() => duplicate(e)}><Copy className="h-4 w-4 mr-1" /> Duplicate</Button>
                <Button size="sm" variant={e.published ? 'secondary' : 'outline'} onClick={() => togglePublish(e.id)}>
                  {e.published ? <><EyeOff className="h-4 w-4 mr-1"/> Unpublish</> : <><CheckCircle2 className="h-4 w-4 mr-1"/> Publish</>}
                </Button>
                <Dialog>
                  <DialogTrigger asChild>
                    <Button size="sm" variant="destructive" className="bg-red-50 text-red-700 hover:bg-red-100 hover:text-red-800"><Trash2 className="h-4 w-4 mr-1" />Delete</Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Delete “{e.title}”?</DialogTitle>
                    </DialogHeader>
                    <p className="text-sm text-gray-600">This action cannot be undone.</p>
                    <DialogFooter>
                      <Button variant="outline">Cancel</Button>
                      <Button onClick={() => handleDelete(e.id)} variant="destructive">Delete</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
